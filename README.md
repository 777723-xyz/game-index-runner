...
`runtime-smoke-test.yml` runs twice per hour as a separate chain. Each batch checks up to 15 own-Pages games with five browser workers and a 30-second per-game deadline. It uses Chrome already installed on GitHub's hosted runner and does not download another browser. A visible nonzero Canvas records `runtimeStatus: playable`; the first timeout only increments `runtimeFailureCount`, and two consecutive failures are required before recording `runtimeStatus: failed`. Games are never deleted or hidden by this check. Results are written to `list.json` and trigger the portal deployment when they change. Playwright still adds some Actions time and network traffic for game loading; use the manual input to run a smaller batch when needed.
