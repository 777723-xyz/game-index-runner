...
const htmlMaxFiles = parsePositiveInt(process.env.HTML_MAX_FILES || "120");
const htmlFetchConcurrency = parsePositiveInt(process.env.HTML_FETCH_CONCURRENCY || "5");
const githubApiTimeoutMs = parsePositiveInt(process.env.GITHUB_API_TIMEOUT_MS || "30000");
...
async function loadHtmlContents(htmlFiles) {
  ...
  async function worker() {
    while (true) {
      const index = cursor++;
      if (index >= htmlFiles.length) return;
  ...
  const workerCount = Math.min(htmlFetchConcurrency, Math.max(1, htmlFiles.length));
  await Promise.all(Array.from({ length: workerCount }, worker));
  return htmlByPath;
}

function compareHtmlCandidates(left, right) {
  const leftName = path.posix.basename(left.path).toLowerCase();
  const rightName = path.posix.basename(right.path).toLowerCase();
  const leftPriority = leftName === "index.html" ? 0 : /^index\d*\.html$/.test(leftName) ? 1 : 2;
  const rightPriority = rightName === "index.html" ? 0 : /^index\d*\.html$/.test(rightName) ? 1 : 2;
  if (leftPriority !== rightPriority) return leftPriority - rightPriority;
  const leftDepth = left.path.split("/").length;
  const rightDepth = right.path.split("/").length;
  return leftDepth - rightDepth || left.path.localeCompare(right.path, "en");
}
...
async function githubRequest(apiPath, options = {}) {
  ...
  const timer = setTimeout(() => controller.abort(), githubApiTimeoutMs);
  let response;
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(`GitHub API request timed out after ${githubApiTimeoutMs}ms: ${apiPath}`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
