...
async function flattenProjectToRoot({ projectRoot, files, htmlByPath, branch, headSha, headCommit }) {
  // If projectRoot is empty, nothing to flatten
  if (!projectRoot || projectRoot === '') {
    return { entryPath: 'index.html', projectRoot: '' };
  }

  console.log(`[flatten] Flattening project from "${projectRoot}" to root`);

  // Collect files to move: all files under projectRoot
  const filesToMove = files.filter(file => file.path.startsWith(projectRoot));
  if (filesToMove.length === 0) {
    console.log(`[flatten] No files found under "${projectRoot}"`);
    return { entryPath: 'index.html', projectRoot: '' };
  }

  // Build a map of new path -> file blob sha
  const updates = new Map();
  const treeEntries = [];

  // For each file to move, get its content and create a new entry at root
  for (const file of filesToMove) {
    const newPath = file.path.slice(projectRoot.length);
    if (newPath === '') continue; // skip directory entry

    // Get blob content
    const blob = await githubRequest(`/repos/${encodeURIComponent(targetOrg)}/${encodeURIComponent(repoName)}/git/blobs/${file.sha}`);
    const content = Buffer.from(blob.content, blob.encoding);
    
    // Create new blob (same content)
    const newBlob = await githubRequest(`/repos/${encodeURIComponent(targetOrg)}/${encodeURIComponent(repoName)}/git/blobs`, {
      method: 'POST',
      body: {
    ...
    // Add to updates map (will be used for commit)
    updates.set(newPath, content);
    
    // Also add to tree entries for deletion of old path
    treeEntries.push({
      path: file.path,
      mode: '100644',
      type: 'blob',
  ...
  // Also delete the projectRoot directory itself (if it's a tree)
  // This is implicitly done by not including it in the new tree.

  // Now commit the changes
  if (updates.size > 0) {
    // Use commitHtmlUpdates with the updates map
    await commitHtmlUpdates({
      branch,
    ...
    console.log(`[flatten] Moved ${updates.size} files from "${projectRoot}" to root`);
  }

  // Update entryPath to root index.html
  const newEntryPath = 'index.html';
  return { entryPath: newEntryPath, projectRoot: '' };
}